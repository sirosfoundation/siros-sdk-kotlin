...
class JweKeystore(
    ...
    override suspend fun unlock(
        ...
        Timber.i("Keystore unlocked with ${keys.size} keys")
    }
    ...
    override suspend fun generateProof(audience: String, nonce: String): String {
        ...
        val key = keys.values.firstOrNull()
            ?: throw KeystoreException("No keys available for proof generation")
    ...
    override suspend fun signPresentation(nonce: String, audience: String, credentialIds: List<String>): String {
        requireUnlocked()
        val key = keys.values.firstOrNull()
            ?: throw KeystoreException("No keys available for VP signing")
    ...
    @Serializable
    private data class KeystoreState(val keys: List<StoredKey>)
